...
class RedisCachingProvider(private val config: CachingProviderConfig?) : CachingProvider<BearerToken>() {
    private var jedis: JedisPooled? = null

    init {
        config?.let {
            val hostAndPort = HostAndPort(config.host, config.port.toInt())
            val jedisConfig = DefaultJedisClientConfig.builder()
    ...
    override fun getTokenForUser(username: String): BearerToken {
        val (token, fingerprint) = parseToken(
